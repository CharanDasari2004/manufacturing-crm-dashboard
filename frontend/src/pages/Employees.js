...
const Employees = () => {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', phone: '', department: '', target: 10
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.post('http://localhost:5000/api/employees', form, config);
    setForm({ name: '', email: '', password: '', phone: '', department: '', target: 10 });
    setShowForm(false);
    fetchEmployees();
  };
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-slate-800 p-6 rounded-xl mb-6 border border-slate-700">
          <h2 className="text-white font-semibold mb-4">Add New Employee</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {employees.map((emp) => (
          <div key={emp._id} className="bg-slate-800 p-6 rounded-xl border border-slate-700">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="bg-slate-700 p-3 rounded-lg text-center">
                ...
                <p className="text-white font-bold">{emp.target}</p>
              </div>
              <div className="bg-slate-700 p-3 rounded-lg text-center">
                <p className="text-slate-400 text-xs">Converted</p>
                <p className="text-green-400 font-bold">{emp.leadsConverted}</p>
              </div>
            </div>
          </div>
        ))}
        {employees.length === 0 && (
          <p className="text-slate-400 col-span-3 text-center py-8">No employees found</p>
        )}
      </div>
    </div>
  );
