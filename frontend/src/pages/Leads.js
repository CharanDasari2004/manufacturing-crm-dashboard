...
const Leads = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (editLead) {
      await axios.put(`http://localhost:5000/api/leads/${editLead._id}`, form, config);
    } else {
      await axios.post('http://localhost:5000/api/leads', form, config);
    }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => { setShowForm(true); setEditLead(null); }}
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-700">
            <tr>
              <th className="text-left text-slate-300 px-6 py-4">Name</th>
              <th className="text-left text-slate-300 px-6 py-4">Company</th>
              <th className="text-left text-slate-300 px-6 py-4">Phone</th>
              <th className="text-left text-slate-300 px-6 py-4">Status</th>
              <th className="text-left text-slate-300 px-6 py-4">Actions</th>
            </tr>
          </thead>
          <tbody>
            {filteredLeads.map((lead) => (
              <tr key={lead._id} className="border-t border-slate-700 hover:bg-slate-750">
                ...
                <td className="px-6 py-4">
                  <span className={`${statusColors[lead.status]} text-white text-xs px-3 py-1 rounded-full`}>
                    {lead.status}
                  </span>
                </td>
                <td className="px-6 py-4">
        ...
        {filteredLeads.length === 0 && (
          <p className="text-slate-400 text-center py-8">No leads found</p>
        )}
      </div>
    </div>
  );
